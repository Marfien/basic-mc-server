...
public class SoundEvent implements Sound.Type {

  // <editor-folder desc="Constants" defaultstate="collapsed">
  ...
  private static final Map<Key, SoundEvent> byName = Maps.newConcurrentMap();
  private static final List<SoundEvent> events = Lists.newArrayList();

  private static synchronized SoundEvent register(final @NotNull String id) {
    final Key name = Key.key(id);
    final SoundEvent event = new SoundEvent(name, events.size());
  ...
  public static @NotNull Optional<SoundEvent> byId(final int id) {
    try {
  ...
  public static @NotNull Optional<SoundEvent> byName(final @NotNull Key name) {
    return Optional.ofNullable(byName.get(name));
  }

  public static @NotNull Packet packet(final @NotNull Sound sound, final @NotNull Vec3D position) {
    final Optional<SoundEvent> event = byName(sound.name());

    return event.isPresent()
        ? new ClientboundSoundPacket(event.get(), sound.source(), position, sound.volume(), sound.pitch())
        : new ClientboundNamedSoundEffectPacket(sound, position);
  }

  private final Key name;
  private final int id;

  private SoundEvent(final @NotNull Key name, final int id) {
    this.name = name;
    this.id = id;
  }
